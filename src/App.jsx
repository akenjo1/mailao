...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      // Check Magic Link restore param
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        // Lắng nghe dữ liệu user (role, limit)
        const userRef = doc(db, 'artifacts', appId, 'users', currentUser.uid, 'profile', 'info');
        
        // Tạo doc nếu chưa có (lần đầu)
        const snap = await getDoc(userRef);
        if (!snap.exists()) {
          await setDoc(userRef, {
            role: 'user', // Mặc định là user, phải chỉnh trong Firebase Console để lên admin
            dailyCount: 0,
  ...
  const handleCreateMailbox = async () => {
    ...
    if (userData.role !== 'admin' && currentCount >= 10) {
      setError("Bạn đã đạt giới hạn 10 mail/ngày. Vui lòng quay lại vào ngày mai.");
      return;
    }
    ...
    try {
      const mailPrefix = generateRandomString(10); // Random prefix
      const fullEmail = `${mailPrefix}@${DOMAIN_NAME}`;
      const apiKey = `key_${generateRandomString(20)}`; // API key giả lập để khôi phục
      ...
      const newMailData = {
        ...
        apiKey: apiKey,
        magicLink: magicLink,
        service: 'Đang chờ...', // Tự động ghi lại khi có mail đến (Logic Backend)
        createdAt: serverTimestamp(),
        restoreCode: magicLinkCode
      };
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.update(userRef, {
          dailyCount: isResetDay ? 1 : increment(1),
          lastResetDate: today
        });

        const historyRef = doc(collection(db, 'artifacts', appId, 'users', user.uid, 'history'));
    ...
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:inset-auto md:flex md:flex-col shadow-xl border-r border-gray-800`}>
      ...
      <nav className="flex-1 p-4 space-y-2">
        <button 
          ...
          className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${view === 'dashboard' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
        >
          <Mail size={20} />
          <span>Hòm thư</span>
        </button>
        <button 
          ...
          className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${view === 'history' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
        >
          <History size={20} />
          <span>Lịch sử</span>
        </button>
        <button 
          ...
          className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${view === 'profile' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
        >
          <User size={20} />
          <span>Tài khoản</span>
        </button>
      ...
      <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-md border border-gray-700">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold py-3 rounded-lg shadow-lg transform transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
          >
           ...
           <div className="space-y-4">
             ...
             <div className="group relative">
               ...
               <div className="flex items-center gap-2 mt-1">
                 ...
                 <button 
                   onClick={() => { navigator.clipboard.writeText(currentMailbox.email); setSuccessMsg("Đã copy Email!"); setTimeout(() => setSuccessMsg(''), 2000); }}
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center text-white">
      <RefreshCw className="animate-spin mr-2" /> Đang khởi động...
    </div>
  );
