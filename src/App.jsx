...
import {
  ...
  orderBy, onSnapshot, updateDoc, serverTimestamp, increment, addDoc
...
// --- HELPER CHUẨN HOÁ ID MAILBOX (ĐỒNG BỘ WORKER) ---
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-900 p-3 rounded border border-gray-700 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <a href={currentMailbox.magicLink} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-white"><ExternalLink size={16} /></a>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  const handleCreateMailbox = async (paramUser = user, paramData = userData) => {
    ...
    try {
      ...
      const emailAddr = `${prefix}@${DOMAIN_NAME}`;
      ...
      // ✅ MAP KEY -> EMAIL (để restore đúng mailbox thật)
      await setDoc(
        doc(db, 'artifacts', APP_ID_DB, 'public', 'keys', newKey),
        {
          email: emailAddr,
          mailboxId: mailboxId(emailAddr),
          createdAt: serverTimestamp(),
        },
        { merge: true }
      );
  ...
  const handleRestoreByKey = async (inputKey) => {
    ...
    try {
      ...
      // ✅ đọc map KEY -> EMAIL
      const keyRef = doc(db, 'artifacts', APP_ID_DB, 'public', 'keys', key);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const mid = mailboxId(currentMailbox.email);
      await addDoc(
        collection(db, 'artifacts', APP_ID_DB, 'public', 'data', 'emails', mid, 'messages'),
        {
          from: 'System Test',
  ...
  if (loading && !user) {
    return <div className="min-h-screen bg-gray-900 flex items-center justify-center text-white"><RefreshCw className="animate-spin mr-2" /> Đang tải...</div>;
  }
