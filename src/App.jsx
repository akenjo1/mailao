      ...
      <div className="mb-2 border-t pt-6">
        ...
        <div className="bg-gray-50 p-6 rounded-xl space-y-4">
          ...
          <div className="flex items-center justify-between">
            <button onClick={handleForgotPass} className="text-sm text-blue-600 hover:underline">Quên mật khẩu? (Gửi link qua Email)</button>
...
function OwnerAdminPanel({ db, user, setUserRoleByUid }) {
  ...
  const findUser = async () => {
    ...
    let snap;
    if (input.includes("@")) {
      snap = await getDocs(query(collection(db, "users"), where("email", "==", input)));
    } else {
...
export default function App() {
  ...
  const setUserRoleByUid = async (targetUid, role) => {
    ...
    if (user.uid !== OWNER_UID) throw new Error("Không có quyền OWNER");
      ...
      <header className="bg-white shadow-sm h-16 fixed w-full top-0 z-50 flex items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-4">
             <span className="hidden md:inline-flex items-center gap-1 text-xs font-medium bg-gray-100 text-gray-600 px-3 py-1 rounded-full border">
                {user && userData?.role === 'admin' ? (<><i className="ph ph-crown-simple text-yellow-600"></i> Admin</>) : (<>User: {guestCount}/10</>)}
             </span>
            ...
            <div className="space-y-6 fade-in">
                ...
                {!currentAddress && (
                     <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 fade-in">
                        ...
                        <div className="flex gap-2">
                            <input type="text" placeholder="Dán mã API Key vào đây..." className="flex-1 p-3 border rounded-lg bg-gray-50 focus:bg-white focus:ring-2 ring-orange-100 outline-none transition" value={restoreKey} onChange={e=>setRestoreKey(e.target.value)} />
                    ...
                    <div className="fade-in space-y-6">
                        <div className="bg-gradient-to-br from-white to-blue-50 border border-blue-100 p-6 rounded-2xl shadow-sm relative overflow-hidden group">
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                ...
                                <div className="bg-white/80 p-3 rounded-lg border border-blue-100 backdrop-blur-sm"><p className="text-xs text-gray-400 font-bold uppercase mb-1">Link truy cập trực tiếp</p><div className="flex items-center justify-between"><a href={`https://${DOMAIN}/?inbox=${apiKey}`} target="_blank" className="text-sm text-blue-500 underline truncate mr-2">Mở Link</a><button onClick={() => { navigator.clipboard.writeText(`https://${DOMAIN}/?inbox=${apiKey}`); alert("Đã copy Link!"); }} className="text-blue-500 hover:text-blue-700"><i className="ph ph-copy"></i></button></div></div>
                            </div>
                        </div>
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 min-h-[400px] flex flex-col">
                            <div className="p-4 border-b flex items-center justify-between bg-gray-50 rounded-t-2xl"><h3 className="font-bold text-gray-700 flex items-center gap-2"><i className="ph ph-tray text-lg text-blue-600"></i> Hộp thư đến</h3><div className="flex items-center gap-3"><div className="flex items-center gap-2 px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>Real-time</div><button onClick={() => window.location.reload()} className="p-2 hover:bg-gray-200 rounded-lg text-gray-500 transition" title="Làm mới"><i className="ph ph-arrows-clockwise text-lg"></i></button></div></div>
