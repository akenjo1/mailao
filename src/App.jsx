  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 text-white transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:inset-auto md:flex md:flex-col shadow-xl border-r border-gray-800`}>
      ...
      <div className="p-4 border-b border-gray-800 bg-gray-800/50">
        ...
        {/* Ô NHẬP API KEY TRONG SIDEBAR */}
        <div className="mt-3 pt-3 border-t border-gray-700">
          <label className="text-[10px] uppercase font-bold text-yellow-500 mb-1 block">Nhập Key Khôi Phục</label>
          <div className="flex gap-1">
            <input 
              type="text" 
              placeholder="Dán mã key..." 
              className="w-full bg-gray-950 text-xs px-2 py-1.5 rounded border border-gray-600 focus:border-yellow-500 outline-none text-white"
              value={sidebarKey}
              onChange={(e) => setSidebarKey(e.target.value)}
            />
            <button 
              onClick={onRestoreSubmit}
              className="bg-yellow-600 hover:bg-yellow-500 text-white px-2 rounded flex items-center justify-center"
            >
              <ArrowRight size={14} />
            </button>
          </div>
      ...
      {currentMailbox ? (
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border border-gray-700 shadow-2xl animate-fade-in">
           ...
           <div className="space-y-4">
             ...
             {/* Magic Link - Có nút mở */}
             <div className="group relative">
               <label className="text-xs text-gray-500 uppercase font-bold tracking-wider">Link truy cập nhanh (Magic Link)</label>
               <div className="flex items-center gap-2 mt-1">
...
export default function App() {
  ...
  // XỬ LÝ MAGIC LINK (Khi vào bằng link ?restore=...)
  useEffect(() => {
    const checkRestoreParam = async () => {
      const urlParams = new URLSearchParams(window.location.search);
      const restoreKey = urlParams.get('restore');
      if (restoreKey) {
        // Tự động kích hoạt khôi phục
        handleRestoreByKey(restoreKey);
        // Xóa param trên thanh địa chỉ
        window.history.replaceState({}, document.title, window.location.pathname);
      }
    };
    // Delay nhẹ để đảm bảo Auth load xong nếu cần
    if (!loading) checkRestoreParam();
  }, [loading]); // Chạy lại khi loading xong
  ...
  const handleRestoreByKey = async (keyInput) => {
    ...
    if (!key || !key.trim()) { 
      if(keyInput) alert("Vui lòng nhập API Key!"); 
      return; 
    }
    setLoading(true);
    try {
      ...
      setView('dashboard');
      setSuccessMsg("Đã khôi phục giao diện mail!");
  ...
  const handleCreateMailbox = async () => {
    ...
    try {
      ...
      const newApiKey = 'key_' + generateRandomString(20);
      const newMailData = {
        email: `${prefix}@${DOMAIN_NAME}`,
        apiKey: newApiKey,
        createdAt: serverTimestamp(),
        magicLink: `${window.location.origin}?restore=${newApiKey}`
      };
